...
public final class ContractsGraph {
    ...
    public void init(Collection<? extends IContractMember> contractMembers) {
        ...
        contractMembers.forEach(contractMember -> {
            if (!Strings.isNullOrEmpty(contractMember.parentId())) {
                Vertex parent = g.V().hasLabel("contract")
                    .has("contract id", contractMember.parentId()).tryNext().orElse(null);
                Vertex contract = g.V().hasLabel("contract")
                    .has("contract id", contractMember.contractId()).tryNext().orElse(null);
                if (parent == null) {
                    log.warn("Parent contract {} doesn't exist", contractMember.parentId());
                    return;
                }
                boolean parentEdgeExists = g.V(parent).outE("parent").where(inV().is(contract)).hasNext();
                if (!parentEdgeExists) {
     ...
     */
    public void addContract(IContractMember contractMember) {
        ...
        Vertex contract = g.V().hasLabel("contract")
            .has("contract id", contractId).tryNext().orElse(null);
        if (contract == null) {
            g.addV("contract")
                .property("data", contractMember)
                .property("contract id", contractId).next();
        } else {
    ...
    public void joinContract(IContractMember contractMember) {
        if (!Strings.isNullOrEmpty(contractMember.contractId()) && !Strings.isNullOrEmpty(contractMember.parentId())) {
    ...
    void printGraph() {
        this.g.V().valueMap().forEachRemaining(
            vertice -> log.info("Vertice: " + vertice)
        );
        log.info("Edges:");
        this.g.E().forEachRemaining(edge -> log.info("Edge: " + edge.label() + " | " +
    ...
    boolean parentExists(String parentId) {
        return this.graph.traversal()
            .V()
            .hasLabel("contract")
